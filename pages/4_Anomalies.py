...
st.caption("Simple heuristics: identify suspicious ratios or outliers. Tune thresholds to your business context.")
...
if {"views_avg", "likes_avg"}.issubset(df.columns):
    ...
    st.dataframe(suspects[["account", "title", "followers", "views_avg", "likes_avg", "like_view_ratio", "kol_score"]].head(300), use_container_width=True)
    ...
    fig, ax = plt.subplots()
    sc = ax.scatter(d["followers"], d["engagement_per_1k_followers"], s=(d["engagement_per_view"]*5000).clip(5, 200), alpha=0.6)
    ax.set_xscale("log"); ax.set_xlabel("Followers (log scale)"); ax.set_ylabel("EP1k")
    st.pyplot(fig)
else:
