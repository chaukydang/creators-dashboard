...
def main():
    ...
    if all(k in resolved for k in ["views_avg","likes_avg","comments_avg","shares_avg"]):
        inter = df[resolved["likes_avg"]].fillna(0) + df[resolved["comments_avg"]].fillna(0) + df[resolved["shares_avg"]].fillna(0)
    ...
    if all(k in resolved for k in ["followers","likes_avg","comments_avg","shares_avg"]):
        inter_f = df[resolved["likes_avg"]].fillna(0) + df[resolved["comments_avg"]].fillna(0) + df[resolved["shares_avg"]].fillna(0)
    ...
    if "likes_avg" in resolved and "views_avg" in resolved:
        df["like_view_ratio"] = df[resolved["likes_avg"]] / df[resolved["views_avg"]].replace(0, np.nan)
    if "comments_avg" in resolved and "views_avg" in resolved:
        df["comment_view_ratio"] = df[resolved["comments_avg"]] / df[resolved["views_avg"]].replace(0, np.nan)
    if "shares_avg" in resolved and "views_avg" in resolved:
        df["share_view_ratio"] = df[resolved["shares_avg"]] / df[resolved["views_avg"]].replace(0, np.nan)

    # chọn & đổi tên cột chuẩn
    keep = []
    for std in ["country","rank","account","title","link","followers","views_avg","likes_avg","comments_avg","shares_avg"]:
        if std in resolved:
            keep.append(resolved[std])
    rename_map = {resolved[k]: k for k in resolved if k in ["country","rank","account","title","link","followers","views_avg","likes_avg","comments_avg","shares_avg"] and resolved.get(k) in keep}

    df_clean = df[keep + ["engagement_per_view","engagement_per_1k_followers","kol_score",
                          "like_view_ratio","comment_view_ratio","share_view_ratio"]].rename(columns=rename_map)
