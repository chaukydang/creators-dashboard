...
with c1: st.metric("KOLs", number_fmt(N))
with c2: st.metric("Top 10% giữ % Reach", kpi_fmt(100*reach_top10p/max(reach_total,1e-9),2))
with c3: st.metric("Top 20% giữ % Reach", kpi_fmt(100*reach_top20p/max(reach_total,1e-9),2))
with c4: st.metric("Median EPV", kpi_fmt(df["engagement_per_view"].median()))
with c5: st.metric("Median EP1k", kpi_fmt(df["engagement_per_1k_followers"].median()))

st.caption("Insight: nếu >70% tổng follower nằm ở top 10–20% KOL → thị trường rất tập trung; chiến lược cần phối hợp Macro/Mega cho awareness + Micro/Mid cho performance.")
...
if x.sum() > 0:
    ...
    ax.set_xlabel("Tỷ lệ KOL (tích lũy)"); ax.set_ylabel("Tỷ lệ Reach (tích lũy)")
    ax.legend(); st.pyplot(fig)

    st.caption("Gini cao → reach tập trung vào ít KOL. Gợi ý: phân bổ ngân sách tránh phụ thuộc vài ‘siêu sao’.")
else:
...
df_dec = df.copy()
df_dec["followers_decile"] = pd.qcut(df_dec["followers"].rank(method="first"), 10, labels=[f"D{i}" for i in range(1,11)]) if df["followers"].notna().sum()>=10 else "All"

grp = df_dec.groupby("followers_decile", observed=False).agg(
...
st.caption("Insight: decile đầu (D10: to nhất) thường có median EP1k thấp hơn; decile giữa (D4-D7) hay là ‘điểm ngọt’ cho performance.")

# ----- 4) Benchmark theo follower tier & country
st.subheader("Benchmark theo Follower Tier")
if "follower_tier" in df.columns:
    bench_tier = df.groupby("follower_tier", observed=False).agg(
        n=("follower_tier","count"),
        epv_med=("engagement_per_view","median"),
        ep1k_med=("engagement_per_1k_followers","median"),
        kol_med=("kol_score","median")
    ).reset_index().sort_values("ep1k_med", ascending=False)
...
if "country" in df.columns and df["country"].notna().any():
    bench_cty = (df.groupby("country")[["engagement_per_view","engagement_per_1k_followers","kol_score"]]
                   .median().sort_values("kol_score", ascending=False).round(4))
...
base_cols = [
    "account","title","followers","views_avg","likes_avg","comments_avg","shares_avg",
    "engagement_per_view","engagement_per_1k_followers","kol_score",
    "like_view_ratio","comment_view_ratio","share_view_ratio","follower_tier","country"
]
...
for name, sub in flags:
    ...
    if any(k in name for k in ["like_view_ratio", "comment_view_ratio", "share_view_ratio"]):
        sort_candidates = [c for c in ["like_view_ratio","comment_view_ratio","share_view_ratio","kol_score"] if c in sub.columns]
    else:
    ...
    if sort_key:
        st.dataframe(sub[display_cols].sort_values(sort_key, ascending=False).head(200), use_container_width=True)
    else:
        st.dataframe(sub[display_cols].head(200), use_container_width=True)

st.info("Cách dùng: Low like/view lặp lại → content/traffic kém chất lượng; High ratios → nội dung rất tốt (nhưng cần kiểm chứng thêm bằng lịch sử/case).")

# Dùng df (Insights) hoặc dff (Leaderboard đã lọc). Ưu tiên dùng dff nếu bạn muốn shortlist theo filter hiện tại.
base = dff if 'dff' in globals() else df
...
if not ok or base[need_cols].dropna().empty:
    ...
    min_like_view = base.get("like_view_ratio", pd.Series([0.0]*len(base))).quantile(0.25) if "like_view_ratio" in base.columns else 0.0

    # 1) Performance-first: EPV & EP1k cùng cao
    perf = base[
        (base["engagement_per_view"] >= thr_epv) &
        (base["engagement_per_1k_followers"] >= thr_ep1k)
    ].copy()
...
tier_msg = ""
if "follower_tier" in df.columns:
    med = (df.groupby("follower_tier")[["engagement_per_view","engagement_per_1k_followers"]]
             .median().sort_values("engagement_per_1k_followers", ascending=False))
    top_tier = med.index[0] if len(med) else None
    tier_msg = f"Nhóm có hiệu quả nổi bật (theo median EP1k) hiện đang là **{top_tier}**." if top_tier else ""
