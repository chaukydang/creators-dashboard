...
with st.sidebar:
    ...
    sort_metric = st.selectbox("Sort by", ["kol_score", "engagement_per_view", "engagement_per_1k_followers", "followers"])

mask = pd.Series(True, index=df.index)
if search:
    s = search.lower()
    cols = []
    if "account" in df.columns: cols.append("account")
    if "title" in df.columns: cols.append("title")
    if cols:
        mask &= df[cols].astype(str).apply(lambda c: c.str.lower().str.contains(s, na=False)).any(axis=1)
if chosen_tiers:
